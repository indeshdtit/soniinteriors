...

{
...

    {
...

        {
...

            {
                ...
                // SAVE EDIT HISTORY
                $edit_history = new \App\Models\Admin\OrderEditHistory();
                $edit_history->order_id = $order->id;
                $edit_history->date_written = $order->date_written;
                $edit_history->customer_name = $order->customer_name;
                $edit_history->pulled_by = $order->pulled_by;
                $edit_history->origination_id = $order->origination_id;
                $edit_history->date_picked_up = $order->date_picked_up;
                $edit_history->status = $order->status;
                $edit_history->order_picked_up_at = $order->order_picked_up_at;
                $edit_history->is_partial_order = $order->is_partial_order;
                $edit_history->shipped_by = $order->shipped_by;
                $edit_history->checked_by = $order->checked_by;
                $edit_history->notes = $order->notes;
...

    {
...

        {
            ...
            Dtit::flash('Order voided successfully.');
...

        {
            ...
            Dtit::flash('Error while voiding order. Please try again.', 'danger');
    ...
    public function undelete_order(Request $request)

    {

        if(Order::modify_order(Route::current()->parameter('order_id'), array(), 'undelete'))

        {

            Dtit::flash('Order un-voided successfully.');
...

        {
            ...
            Dtit::flash('Error while un-voiding order. Please try again.', 'danger');
    ...
    {
        if (base64_encode(Route::current()->parameter('pwd')) == Auth::user()->rempwd)
        {
            if(Order::modify_order(Route::current()->parameter('order_id'), array(), 'permanent_delete'))
            {
        ...
        {
            Dtit::flash('Unauthorized request. Password that you entered is incorrect.', 'danger');
        }
